...
ALTER TABLE Manga DROP CONSTRAINT fkMangaId;
ALTER TABLE Anime DROP CONSTRAINT fkAnimeId;

ALTER TABLE Manga
    ADD CONSTRAINT fkMangaId FOREIGN KEY(MediumId) REFERENCES Medium(Id) ON DELETE CASCADE;
ALTER TABLE Anime
    ADD CONSTRAINT fkAnimeId FOREIGN KEY(MediumId) REFERENCES Medium(Id) ON DELETE CASCADE;


SELECT * FROM MEDIUM;
SELECT * FROM ACCOUNT;
UPDATE ACCOUNT
    SET imagelink = NULL WHERE "Id" = 61;
Commit;
...
FROM 
    ALL_CONSTRAINTS
WHERE 
    OWNER = 'SBD155910' AND CONSTRAINT_NAME LIKE 'FK%'
ORDER BY 
